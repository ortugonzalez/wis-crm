...
create table if not exists secretary_preferences (
  ...
  display_name text not null default 'Ortu',
  role_title text not null default 'Founder',
  monthly_intent text,
  current_focus text,
  preferred_segments jsonb not null default '[]'::jsonb,
  priority_channels jsonb not null default '["whatsapp","linkedin","email"]'::jsonb,
  energy_mode text not null default 'equilibrado',
  current_state text not null default 'arrancando',
  coaching_style text not null default 'concreto',
  relationship_goal text,
  context_notes text,
  daily_contact_target integer not null default 10,
  daily_followup_target integer not null default 3,
  daily_proposal_target integer not null default 1,
  one_thing_rule boolean not null default true,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  ...
  constraint secretary_preferences_energy_mode_check check (energy_mode in ('suave', 'equilibrado', 'intenso')),
  constraint secretary_preferences_current_state_check check (current_state in ('arrancando', 'en_foco', 'saturado', 'trabado', 'cerrando')),
  constraint secretary_preferences_coaching_style_check check (coaching_style in ('concreto', 'amable', 'exigente')),
  constraint secretary_preferences_targets_check check (daily_contact_target >= 0 and daily_followup_target >= 0 and daily_proposal_target >= 0)
);
...
alter table secretary_preferences add column if not exists display_name text not null default 'Ortu';
alter table secretary_preferences add column if not exists role_title text not null default 'Founder';
alter table secretary_preferences add column if not exists monthly_intent text;
alter table secretary_preferences add column if not exists current_focus text;
alter table secretary_preferences add column if not exists preferred_segments jsonb not null default '[]'::jsonb;
alter table secretary_preferences add column if not exists priority_channels jsonb not null default '["whatsapp","linkedin","email"]'::jsonb;
alter table secretary_preferences add column if not exists energy_mode text not null default 'equilibrado';
alter table secretary_preferences add column if not exists current_state text not null default 'arrancando';
alter table secretary_preferences add column if not exists coaching_style text not null default 'concreto';
alter table secretary_preferences add column if not exists relationship_goal text;
alter table secretary_preferences add column if not exists context_notes text;
alter table secretary_preferences add column if not exists daily_contact_target integer not null default 10;
alter table secretary_preferences add column if not exists daily_followup_target integer not null default 3;
alter table secretary_preferences add column if not exists daily_proposal_target integer not null default 1;
alter table secretary_preferences add column if not exists one_thing_rule boolean not null default true;
